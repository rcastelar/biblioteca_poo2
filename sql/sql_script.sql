...
CREATE TABLE IF NOT EXISTS `biblioteca`.`livro` (
  ...
  PRIMARY KEY (`id`, `editora_id`, `sub_divisao_id`),
  ...
  CONSTRAINT `fk_livro_sub_divisao1`
    FOREIGN KEY (`sub_divisao_id`)
    REFERENCES `biblioteca`.`sub_divisao` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `fk_livro_sub_divisao1_idx` ON `biblioteca`.`livro` (`sub_divisao_id` ASC);
...
CREATE TABLE IF NOT EXISTS `biblioteca`.`exemplar` (
  `livro_id` INT NOT NULL,
  `codigo_exemplar` VARCHAR(45) NOT NULL,
...
CREATE INDEX `fk_exemplar_livro1_idx` ON `biblioteca`.`exemplar` (`livro_id` ASC);

CREATE UNIQUE INDEX `codigo_exemplar_UNIQUE` ON `biblioteca`.`exemplar` (`codigo_exemplar` ASC);
...
CREATE TABLE IF NOT EXISTS `biblioteca`.`emprestimos` (
  `usuario_id` INT NOT NULL,
  `codigo_exemplar` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`usuario_id`, `codigo_exemplar`),
  CONSTRAINT `fk_emprestimos_usuario1`
    FOREIGN KEY (`usuario_id`)
    REFERENCES `biblioteca`.`usuario` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_emprestimos_exemplar1`
    FOREIGN KEY (`codigo_exemplar`)
    REFERENCES `biblioteca`.`exemplar` (`codigo_exemplar`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_emprestimos_exemplar1_idx` ON `biblioteca`.`emprestimos` (`codigo_exemplar` ASC);
